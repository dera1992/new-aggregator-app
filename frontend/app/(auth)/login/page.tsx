...
export default function LoginPage() {
  ...
  const resendMutation = useMutation({
    ...
    onError: () => toast.error('Could not resend confirmation email'),
  });
  ...
  const mutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      if (isUnconfirmed) {
        setUnconfirmedEmail(form.getValues('email'));
      } else {
    ...
    <div className="space-y-6">
      ...
      {unconfirmedEmail && (
        <div className="rounded-md border border-yellow-500/30 bg-yellow-500/10 p-4 text-sm">
          <p className="font-medium text-yellow-600 dark:text-yellow-400">Email not confirmed</p>
      ...
      <a
        ...
        className="flex w-full items-center justify-center gap-2 rounded-md border border-input bg-background px-3 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground transition-colors"
      >
