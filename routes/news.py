...
def get_personalized_feed():
    ...
    # Load stored preferences if they exist, otherwise default to empty filters.
    ...
    # Group articles by cluster into a story list.
    stories = {}
    for a in articles:
        cid = a.cluster_id
        if cid not in stories:
...
def save_article():
    ...
    # Avoid duplicate saves for the same user/article pair.
...
def list_saved_articles():
    # Fetch saved articles in reverse chronological order.
    saved_entries = (
...
def list_read_articles():
    # Fetch read articles in reverse chronological order.
    read_entries = (
