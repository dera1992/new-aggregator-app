...
def _normalize_list(value):
    """Normalize list inputs into a stripped list of strings."""
    if value is None:
        return []
...
def _is_valid_digest_time(value):
    """Validate digest time strings in HH:MM 24h format."""
    if value is None:
        return True
...
def _get_or_create_preferences(user_id):
    """Fetch preferences or create a default record for new users."""
