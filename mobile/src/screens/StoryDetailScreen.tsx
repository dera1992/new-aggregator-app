...
import { PerspectiveTab } from "@/screens/story/tabs/PerspectiveTab";
...
export function StoryDetailScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"compose" | "perspective">(
    "compose",
  );
  ...
  const analysisResult = analysisMutation.data as
    | GenerateAnalysisResponse
    | undefined;
  ...
  const analysisCopy = (
    variant: GenerateAnalysisResponse["variants"][number],
  ) => {
    return [
      ...
      <View className="gap-6">
        <Card className="gap-3">
          ...
          <View className="gap-2">
            <Text className="text-xs font-semibold text-muted-foreground dark:text-dark-muted-foreground">
              Sources
            </Text>
        ...
        <SegmentedControl
          ...
          value={activeTab}
          onChange={(value) => setActiveTab(value as "compose" | "perspective")}
        />
        ...
        {activeTab === "compose" ? (
            ...
            <Accordion title="Viral Post" defaultOpen>
              <View className="gap-4">
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setViralOptions((prev) => ({ ...prev, platform: value }))
                  }
                />
                <Input
                  placeholder="Tone"
                  ...
                  onChangeText={(value) =>
                    setViralOptions((prev) => ({ ...prev, tone: value }))
                  }
                />
                <Input
                  placeholder="Goal"
                  value={viralOptions.goal}
                  onChangeText={(value) =>
                    setViralOptions((prev) => ({ ...prev, goal: value }))
                  }
                />
                <Input
                  placeholder="Audience"
                  value={viralOptions.audience}
                  onChangeText={(value) =>
                    setViralOptions((prev) => ({ ...prev, audience: value }))
                  }
                />
                <Input
                  placeholder="Brand voice"
                  value={viralOptions.brand_voice}
                  onChangeText={(value) =>
                    setViralOptions((prev) => ({ ...prev, brand_voice: value }))
                  }
                />
                <Input
                ...
                {viralMutation.error ? (
                  <ErrorState
                    message={(viralMutation.error as Error).message}
                  />
                ) : null}
                {viralResult?.warnings?.length ? (
                  <Text className="text-xs text-yellow-600">
                    Warnings: {viralResult.warnings.join(", ")}
                  </Text>
                ) : null}
            ...
            <Accordion title="Comment">
              <View className="gap-4">
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setCommentOptions((prev) => ({ ...prev, platform: value }))
                  }
                />
                <OptionGroup
                  label="Style"
                  ...
                  onChange={(value) =>
                    setCommentOptions((prev) => ({ ...prev, style: value }))
                  }
                />
                <Input
                  placeholder="Audience"
                  value={commentOptions.audience}
                  onChangeText={(value) =>
                    setCommentOptions((prev) => ({ ...prev, audience: value }))
                  }
                />
                <Input
                  placeholder="Max variants"
                ...
                {commentMutation.error ? (
                  <ErrorState
                    message={(commentMutation.error as Error).message}
                  />
                ) : null}
                {commentResult?.warnings?.length ? (
                  <Text className="text-xs text-yellow-600">
                    Warnings: {commentResult.warnings.join(", ")}
                  </Text>
                ) : null}
            ...
            <Accordion title="Joke">
              <View className="gap-4">
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setJokeOptions((prev) => ({ ...prev, platform: value }))
                  }
                />
                <OptionGroup
                  label="Style"
                  ...
                  onChange={(value) =>
                    setJokeOptions((prev) => ({ ...prev, style: value }))
                  }
                />
                <Input
                  placeholder="Audience (optional)"
                  value={jokeOptions.audience}
                  onChangeText={(value) =>
                    setJokeOptions((prev) => ({ ...prev, audience: value }))
                  }
                />
                <Input
                  placeholder="Max variants"
                  ...
                  onChangeText={(value) =>
                    setJokeOptions((prev) => ({ ...prev, max_variants: value }))
                  }
                />
                <OptionGroup
                  label="Fact mode"
                ...
                <Button
                  label={
                    jokeMutation.isPending ? "Generating..." : "Generate joke"
                  }
                  onPress={() => jokeMutation.mutate()}
                  disabled={!summaryReady}
                />
                {jokeMutation.error ? (
                  <ErrorState message={(jokeMutation.error as Error).message} />
                ) : null}
                {jokeResult?.warnings?.length ? (
                  <Text className="text-xs text-yellow-600">
                    Warnings: {jokeResult.warnings.join(", ")}
                  </Text>
                ) : null}
            ...
            <Accordion title="Analysis">
              <View className="gap-4">
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setAnalysisOptions((prev) => ({ ...prev, format: value }))
                  }
                />
                <OptionGroup
                  label="Tone"
                  ...
                  onChange={(value) =>
                    setAnalysisOptions((prev) => ({ ...prev, tone: value }))
                  }
                />
                <OptionGroup
                  label="Audience"
                  ...
                  onChange={(value) =>
                    setAnalysisOptions((prev) => ({ ...prev, audience: value }))
                  }
                />
                <OptionGroup
                ...
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setAnalysisOptions((prev) => ({
                      ...prev,
                      include_counterpoints: value === "yes",
                    }))
                  }
                />
                <OptionGroup
                  ...
                  onChange={(value) =>
                    setAnalysisOptions((prev) => ({
                      ...prev,
                      include_what_to_watch: value === "yes",
                    }))
                  }
                />
                <OptionGroup
                ...
                {analysisMutation.error ? (
                  <ErrorState
                    message={(analysisMutation.error as Error).message}
                  />
                ) : null}
          ...
          <PerspectiveTab clusterId={Number(clusterId)} />
        )}
      </View>
    </Screen>
  );
